...
ANSWER: each row will be represent an accident.
...
ANSWER: The data showed how many accidents happened throughout the week. Based on the plot we can say that there are no fatal accident during the weekend. And suprisingly most of the accident that occured from 12am to 6am during weekday are most likely to be fatal. 

```{r accident_severe_throughout_the_week, eval = TRUE, echo = FALSE, fig.fullwidth = TRUE}
accidents %>%
...
```{r Road_Condition, eval = TRUE, echo = FALSE, fig.fullwidth = TRUE}
accidents %>%
  filter(day_of_week %in% c("Saturday", "Sunday")) %>%
  ggplot(aes(x = casualties, fill = severity)) +
  geom_density(alpha = 0.5) +
